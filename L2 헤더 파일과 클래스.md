...
```
이 코드에 문제가 있는 것은 아니다. 이것뿐이라면 잘 동작한다. 그러나 bar.h에 `#include "foo.h"`가 있다. 이것은 권장할 수 없다. 문제가 발생할 수 있기 때문이다. 어떤 경우에 문제가 발생할까.

계획이 바뀌어 클래스 `Foo`가 `Bar`의 인스턴스를 필드로 가진다고 하자. 그러면 foo.h는 다음과 같이 수정되어야 한다.
